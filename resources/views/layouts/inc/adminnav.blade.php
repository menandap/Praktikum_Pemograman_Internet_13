<div class="container-fluid py-1 px-3">
        ...
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          ...
          @php $admin_unRead = App\Models\Admin_notifications::where('notifiable_id',auth()->user()->id)->where('read_at', NULL)->orderBy('created_at','desc')->count(); @endphp         
            <li class="nav-item dropdown d-flex align-items-center">
              ...
              <ul class="dropdown-menu dropdown-menu-end px-2 me-sm-n4" aria-labelledby="dropdownProfileButton">
                @php $admin_notifikasi = App\Models\Admin_notifications::where('notifiable_id',auth()->user()->id)->where('read_at', NULL)->orderBy('created_at','desc')->get(); @endphp
                @forelse ($admin_notifikasi as $notifikasi)
                @php $notif = json_decode($notifikasi->data); @endphp
                <li>
                    <a href="{{ route('admin.notification', $notifikasi->id) }}" class="dropdown-item btnunNotif" data-num=""><small> [User: {{ $notif->nama }}] {{ $notif->message }}</small></a>
                </li>
                @empty
                    <li>
